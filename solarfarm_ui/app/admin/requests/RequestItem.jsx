...
export default function RequestItem({ request }) {
    ...
    const handleAuthorize = async () => {
        ...
        try {
            ...
            const authRequest = new AuthorizationRequest(request.userId, request.ethereumAddress, request.metadata);
    ...
    return (
        <div className="bg-white shadow-md rounded-lg p-6 m-4 max-w-md">
            ...
            <div className="text-sm text-gray-600 space-y-1">
                ...
                <p>
                    <strong>Email:</strong> {request.metadata.email}
                </p>
                <p>
                    <strong>Reason:</strong> {request.metadata.reason}
                </p>
                <p>
                    <strong>Timestamp:</strong> {new Date(request.metadata.timestamp).toLocaleString()}
                </p>
                <p>
                    <strong>Status:</strong>{" "}
                    <span
                        className={`font-medium ${status === "approved" ? "text-green-600" : status === "rejected" ? "text-red-600" : status === "failed" ? "text-red-600" : "text-yellow-600"}`}
                    >
                        {status}
                    </span>
                </p>
