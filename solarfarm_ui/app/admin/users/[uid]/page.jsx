...
export default function DetailsPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        setAuthorized(
          await checkIfAuthorizedAction(userData.toJSON(), csrfData.token)
        );
      } else {
        setUsers(null);
      }
  ...
  const toggleAuherization = async () => {
    ...
    try {
      ...
      setAuthorized(await checkIfAuthorizedAction(currUser.toJSON(), csrf));
    } catch (error) {
      console.log(error);
    }
  };
