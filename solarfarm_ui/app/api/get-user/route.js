...
export async function GET(req) {
  try {
    ...
    if (userData.uraniumLbs < 0) {
      console.warn(
        `Invalid uranium value for user ${uid}: ${userData.uraniumLbs}`
      );
      userData.uraniumLbs = 0; // Reset to 0 to prevent invalid commits
    }

    if (
      userData.ethereumAddress &&
      !ethers.isAddress(userData.ethereumAddress)
    ) {
      console.warn(
        `Invalid Ethereum address for user ${uid}: ${userData.ethereumAddress}`
      );
    ...
    const response = new User({
      ...
      uraniumLbs: userData.uraniumLbs || 0,
