...
const getContractWithPrivateKey = async () => {
  try {
    ...
    if (network.chainId.toString() !== targetConfig.chainId) {
      throw new Error(
        `Wrong network. Expected ${targetConfig.chainId}, got ${network.chainId}`
      );
    }
    ...
    const errorMessage = handleContractError(
      error,
      "server-side contract initialization"
    );
    throw new Error(
      `Failed to initialize contract with private key: ${errorMessage}`
    );
  }
};

// Modified gas estimation function for server-side use
export const estimateGasCostForCommitment = async (
  amount,
  user,
  secret,
  csrfToken
  ...
  try {
    ...
    if (amount > 1000) {
      // Updated: Changed error message from kWh to lbs for consistency
      throw new Error("Amount cannot be bigger than 1000 lbs");
    }
    ...
    const hash = getHashedCommitment(amount, nonce, walletAddress, secret);
    ...
    return {
      ...
      data: {
        ...
        walletAddress: contract.runner.address,
        secret: secret,
      },
    };
  } catch (error) {
...
export async function estimateCommitmentGas(formData) {
  try {
    ...
    const secret = ethers.hexlify(ethers.randomBytes(32)); // Random 32-byte secret
    if (!amount || !userUid || !userEthereumAddress) {
      throw new Error(
        "Missing required parameters: amount, userUid, userEthereumAddress"
      );
    }
    ...
    const result = await estimateGasCostForCommitment(
      amount,
      user,
      secret,
      csrfToken
...
export const estimateGasCostForRevealPurchase = async (
  amount,
  user,
  secret,
  csrfToken
...
export async function estimateRevealGas(formData) {
  try {
    ...
    console.log("estimateRevealGas secret:", secret);
    if (!amount || !userUid || !userEthereumAddress) {
      throw new Error(
        "Missing required parameters: amount, userUid, userEthereumAddress"
      );
    ...
    const result = await estimateGasCostForRevealPurchase(
      amount,
      user,
      secret,
      csrfToken
...
export const saveOrderToFirebase = async (order, user, csrfToken, uid) => {
  validateCsrfToken(csrfToken);

  await saveData(order, `committedOrders/${uid}/${order.transactionHash}`);
  user.uraniumLbs += order.uraniumAmountLbs;
  await saveData(user, `users/${uid}`);
};

export const checkIfAuthorizedAction = async (user, csrfToken) => {
  try {
    console.log("checkIfAuthorizedAction user:", csrfToken);
    validateCsrfToken(csrfToken);
