...
import {
  ...
  saveRequest,
  updateUser,
} from "../actions/usersContractActions";
    ...
    try {
      ...
      if (user.ethereumAddress) {
        const response = await fetch(
          "/api/get-eth-balance?address=" + user.ethereumAddress,
          {
            method: "GET",
            headers: {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        setCsrfToken(token);
        setIsAuthorized(
          await checkIfAuthorizedAction(userModel.toJSON(), token)
        );
    ...
    <div className="items-center flex justify-self-center">
      <Card title="User Profile" maxHeight="max-h-[600px]">
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <p>
            <span className="font-medium text-primary-600">Uranium:</span>{" "}
            <span className="text-secondary-900">
              {currUser.uraniumLbs ? `${currUser.uraniumLbs} Lbs` : "0 Lbs"}
            </span>
          </p>
          <p>
