...
contract EnergyContract is Ownable, Pausable, ReentrancyGuard {
    ...
    event EnergyPurchased(address indexed buyer, address indexed seller, uint256 indexed kWh, uint256 totalCostWei, uint256 ethPriceUSD, uint256 timestamp);
    ...
    function confirmAddEnergy(uint256 _kWh) external onlyOwner whenNotPaused {
        ...
        if (block.timestamp < lastAddEnergyRequest[msg.sender] + ADD_ENERGY_DELAY) revert DelayNotElapsed(lastAddEnergyRequest[msg.sender], block.timestamp);
        if (_kWh == 0 || _kWh > MAX_KWH_PER_PURCHASE) revert InsufficientEnergyAvailable(_kWh, MAX_KWH_PER_PURCHASE);
        availableKWh += _kWh;
    ...
    function commitPurchase(bytes32 _commitmentHash) external onlyAuthorizedParties whenNotPaused {
        ...
        if (block.timestamp < lastCommitTime[msg.sender] + COMMIT_COOLDOWN) revert CommitmentCooldownActive(lastCommitTime[msg.sender], block.timestamp);
    ...
    function revealPurchase(uint256 _kWh, uint256 _nonce) external payable onlyAuthorizedParties whenNotPaused nonReentrant {
        if (_kWh == 0 || _kWh > MAX_KWH_PER_PURCHASE || availableKWh < _kWh) revert InsufficientEnergyAvailable(_kWh, availableKWh);
        ...
        if (block.timestamp > commitment.timestamp + COMMIT_REVEAL_WINDOW) revert CommitmentExpired(commitment.timestamp, block.timestamp);
        if (keccak256(abi.encodePacked(_kWh, _nonce, msg.sender)) != commitment.commitmentHash) revert InvalidCommitment();
    ...
    function updatePaymentReceiver(address _newReceiver) external onlyOwner {
        ...
        uint256 size;
        assembly { size := extcodesize(_newReceiver) }
...
function getLatestEthPrice() public returns (uint256) {
    ...
    function calculateRequiredPayment(uint256 _kWh, uint256 _ethPriceUSD) external pure returns (uint256) {
        if (_kWh == 0 || _kWh > MAX_KWH_PER_PURCHASE) revert InsufficientEnergyAvailable(_kWh, MAX_KWH_PER_PURCHASE);
